...
public class FilesUtils {
    ...
    public static void readAll2Conig(String filePathAndName, String encoding, Map<String, Object> conifg) {
        FileInputStream fileInputStream = null;
        try {
            fileInputStream = new FileInputStream(URLDecoder.decode(filePathAndName, encoding));
            InputStreamReader inputStreamReader;
            ...
            try {
                ...
                conifg.put("holidayList", holidayList);
                conifg.put("weekdayList", weekdayList);
                BufferedReader bufferedReader = new BufferedReader(inputStreamReader);
                String current = "";
                while ((data = bufferedReader.readLine()) != null) {
                    if (data.trim().startsWith("#") || "".equals(data.trim())) {
                        continue;//注释行和空行跳过
                    }
                    if ("holiday".equals(data.trim())) {
                        current = "holiday";//节假日
                        continue;
                    } else if ("weekday".equals(data.trim())) {
                        current = "weekday";//调休
                        continue;
                    }
                    if ("holiday".equals(current)) {
                        holidayList.add(data.trim());
                    } else if ("weekday".equals(current)) {
                        weekdayList.add(data.trim());
                    }
                }
            } catch (Exception e) {
            }
